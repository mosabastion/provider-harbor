...
// returning (nil, nil) when absent. Used to adopt a member whose Harbor id is not
// yet recorded as the external name.
...
func (c *HarborClient) findProjectMember(ctx context.Context, projectID, username string) (*harbormodels.ProjectMemberEntity, error) {
	return c.findProjectMemberByEntity(ctx, projectID, "u", username)
}

// AddProjectMember adds a user member to a Harbor project with the given role.
//
// Deprecated: serves the deprecated user-only Member kind. It delegates to the
...
// UpdateProjectMember updates a project member's role by username.
func (c *HarborClient) UpdateProjectMember(ctx context.Context, projectID, username, role string) error {
	...
	return c.UpdateProjectMemberByID(ctx, projectID, strconv.FormatInt(m.ID, 10), role)
}

// DeleteProjectMember removes a member from a project by username (idempotent).
func (c *HarborClient) DeleteProjectMember(ctx context.Context, projectID, username string) error {
	...
	return c.DeleteProjectMemberByID(ctx, projectID, strconv.FormatInt(m.ID, 10))
}
